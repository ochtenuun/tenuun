...
function StudentCard({ student, onDelete }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow hover:shadow-lg transition-all duration-200 p-4 sm:p-5 flex flex-col gap-3 w-full">
      {/* Avatar */}
      <div className="flex justify-center">
        {image && !imgError ? (
          <img
            ...
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover border-4 border-gray-100 shadow"
          />
        ) : (
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center text-xl sm:text-2xl font-bold text-gray-500 border-4 border-gray-100 shadow">
            {name[0]}
          </div>
        )}
      </div>
                ...
                <div key={i} className="flex flex-col items-center gap-1">
                  {itemImage ? (
                    <img
                      src={itemImage}
                      alt={itemName}
                      className="w-9 h-9 sm:w-10 sm:h-10 object-cover rounded-lg border border-gray-200"
                    />
                  ) : (
                    <div className="w-9 h-9 sm:w-10 sm:h-10 bg-gray-100 rounded-lg flex items-center justify-center text-base sm:text-lg">
                      🖱️
                    </div>
                  )}
      ...
      <button
        ...
        className="mt-auto w-full bg-red-500 hover:bg-red-600 active:scale-95 text-white text-xs sm:text-sm font-bold py-2 rounded-xl transition-all duration-150 cursor-pointer"
      >
        Delete
      </button>
    </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center mb-4 sm:mb-8">
          <input
            ...
            className="w-full max-w-xl border-2 border-blue-400 focus:border-blue-600 outline-none rounded-lg px-4 py-2 sm:py-2.5 text-gray-700 bg-white shadow-sm text-sm transition-colors"
          />
        </div>
